...
export class ConversationController {
  ...
  getGreeting(): string {
    ...
    const dayGreetings: Record<number, string[]> = {
      0: [personalize(t('chat.greeting.day.sunday')), t('chat.greeting.day.sundaySession'), t('chat.greeting.day.sundayWeekend')],
      1: [personalize(t('chat.greeting.day.monday')), personalize(t('chat.greeting.day.mondayBack'), t('chat.greeting.day.mondayBack'))],
      2: [personalize(t('chat.greeting.day.tuesday'))],
      3: [personalize(t('chat.greeting.day.wednesday'))],
      4: [personalize(t('chat.greeting.day.thursday'))],
      5: [personalize(t('chat.greeting.day.friday')), personalize(t('chat.greeting.day.fridayFeeling'))],
      6: [personalize(t('chat.greeting.day.saturday'), t('chat.greeting.day.saturday')), personalize(t('chat.greeting.day.saturdayWeekend'))],
    };

    // Time-specific greetings
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize(t('chat.greeting.time.morning')), t('chat.greeting.time.morningCoffee')];
      } else if (hour >= 12 && hour < 18) {
        return [personalize(t('chat.greeting.time.afternoon')), personalize(t('chat.greeting.time.hey')), t('chat.greeting.time.howsItGoing')];
      } else if (hour >= 18 && hour < 22) {
        return [personalize(t('chat.greeting.time.evening')), personalize(t('chat.greeting.time.eveningShort')), t('chat.greeting.time.howWasYourDay')];
      } else {
        return [t('chat.greeting.time.nightOwl'), personalize(t('chat.greeting.time.eveningShort'))];
      }
    };

    // General greetings
    const generalGreetings = [
      personalize(t('chat.greeting.general.hey')),
      name ? t('chat.greeting.general.hiNameHow', { name }) : t('chat.greeting.general.hiHow'),
      t('chat.greeting.general.howsItGoing'),
      t('chat.greeting.general.welcomeBack'),
      t('chat.greeting.general.whatsNew'),
      ...(name ? [t('chat.greeting.general.nameReturns', { name })] : []),
      t('chat.greeting.general.youAreRight'),
    ];
