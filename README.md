...
- **데이터베이스 연결**: MySQL/ClickHouse DB를 연결하면 Claude가 자연어로 데이터를 조회할 수 있다. 여러 연결을 등록하고 활성화/비활성화할 수 있으며, SELECT/SHOW/DESCRIBE만 허용하고 쓰기 구문(INSERT/UPDATE/DELETE/DROP 등)은 차단한다. `db.table` 형식으로 크로스 DB 쿼리도 가능하다.
- **채팅 글자 크기**: 설정에서 채팅 대화창의 글자 크기를 xs~xl 5단계로 조절할 수 있다.
...
**커스터마이징**
- **채팅 글자 크기**: 채팅 대화창의 글자 크기 (xs / sm / md / lg / xl)
...
**데이터베이스**
- MySQL / ClickHouse 연결 추가/편집/삭제; 연결별 활성화/비활성화; 연결 테스트
- 활성화된 연결은 MCP 서버로 자동 등록되어 Claude가 자연어로 데이터 조회 가능
- DB를 지정하지 않으면 서버 전체에 접근해 크로스 DB 쿼리 가능
- 쓰기 구문 차단: INSERT / UPDATE / DELETE / DROP / TRUNCATE / ALTER 등
...
│   ├── images/                  # 이미지 캐싱 및 로딩
│   ├── db/                      # DB 연결 (MySqlClient, ClickHouseClient, DbMcpServer, DbManager)
...
| 버전 | 변경 내용 |
|------|-----------|
| **1.3.5** | DB 연결 — MySQL/ClickHouse를 플러그인에서 직접 연결해 Claude가 자연어로 데이터 조회. 조회 전용(SELECT/SHOW/DESCRIBE), 쓰기 차단, 크로스 DB 쿼리 지원 |
| **1.3.4** | 채팅 글자 크기 xs~xl 5단계 조절 |
| **1.3.3** | 마스코트 버그 수정 및 인터랙션 개선 |
| **1.3.2** | 설정 탭 네비게이션 UX 개선 |
| **1.3.1** | 컨텍스트 버그 수정 |
| **1.3.0** | 파일 컨텍스트 UX 강화 — @멘션 파일 칩 패널 (모든 첨부 파일 한눈에 표시), 파일 크기 힌트 (줄 수 자동 표시), .md 파일 드래그 앤 드롭 (최대 10개), 우클릭 메뉴 컨텍스트 추가/제거, 3개 초과 시 접기/펼치기 |
| **1.2.0** | 디자이너 도구 + UX 강화 — 디자인 슬래시 커맨드 15개, 디자인 커스텀 에이전트 7개, 사이드바 테마 6종, 로딩 애니메이션 5종, 시간대/계절 반응 UI, 픽셀 아트 마스코트 6종, 뽀모도로/디자인 스프린트 타이머, 일일 디자인 인사이트, 커맨드/에이전트 업데이트 동기화 |
...
- [x] 디자이너 도구 (슬래시 커맨드 15개 + 에이전트 7개)
- [x] UX Experience (테마, 로딩, 시간대/계절, 마스코트)
- [x] 뽀모도로 / 디자인 스프린트 타이머
- [x] 파일 컨텍스트 UX (칩 패널, 줄 수 힌트, 드래그 앤 드롭, 우클릭 메뉴)
- [x] MySQL / ClickHouse DB 연결 (자연어 조회, 조회 전용)
- [x] 채팅 글자 크기 조절
- [ ] 훅 및 기타 고급 기능
- [ ] 더 많은 기능 예정!
