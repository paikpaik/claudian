...
describe('ClaudianService', () => {
  ...
  describe('persistent query dynamic updates', () => {
    ...
    it('updates permission mode via setPermissionMode when going from auto to normal', async () => {
      // Start in auto mode
      mockPlugin.settings.permissionMode = 'auto';
      service = new ClaudianService(mockPlugin, createMockMcpManager());
      ...
      const response = getLastResponse();
      // Should call setPermissionMode for auto -> normal transition
      expect(response?.setPermissionMode).toHaveBeenCalledWith('acceptEdits');
    });

    it('updates permission mode via setPermissionMode when going from normal to auto', async () => {
      ...
      const response = getLastResponse();
      // Should call setPermissionMode for normal -> auto transition (no restart needed)
